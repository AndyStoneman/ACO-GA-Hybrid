...
public class AGHybrid {
    ...
    /**
     * A final constant variable that determines the amount that pheromone evaporates each iteration. 
     */
    private static final double ROW = 0.5;
    /**
     * A final constant variable that determines the importance of the pheromone levels in calculating
     * probabilities. 
     */
    ...
    /**
     * A 2d array to hold the pheromone levels between each city. Mirrored over the diagonal.
     */
     ...
     * which denotes how a city is picked for a given iteration. Updated every iteration when the pheromone 
     * is updated. 
     */
    private double[][] probabilitiesList;
    /**
     * List used to hold all twenty ants that will be used to construct tours. 
     */
    private Ant[] antCollection;
    /**
     * String that denotes the crossover type. Two options are 'OX' and 'PMX'.
     */
     ...
     */
    public AGHybrid(String fileName, double optTour, boolean percentageCutoff, int numIterations, double targetCutoff, String crossoverType) {
    ...
    private void initializePheromoneLevels() {
        ...
        for (int i = 0; i < numCities; i++) {
            for (int j = i; j < numCities; j++) {
                if (i == j) {
                    pheromoneLevels[i][j] = 0.0;
                    pheromoneLevels[j][i] = 0.0;
                } else {
                    pheromoneLevels[i][j] = NUM_ANTS / total;
                    pheromoneLevels[j][i] = NUM_ANTS / total;
                }
            }
        }
    }
     ...
     */
    public void solve() {
        System.out.println("\nSTARTING ACO-GA HYBRID ALGORITHM\n");
        ...
        //implement a 2d pheromone array that holds a specified start value
        initializePheromoneLevels();
        
        //create collection of ants
        antCollection = new Ant[NUM_ANTS];
        ...
        while (iteration < numIterations) {
            //calculate new probabilities for each leg based on new pheromone every iteration
            calculateProbabilities();
            
            //iterate through ants, creating a tour for each of them
            constructTours();
            
            //run ga to modify ant tours before updating pheromone
            gaUpdate();
            
            //after each iteration, update pheromone levels and calculate best tour of iteration
            double bestTour = updatepheromoneLevels(antCollection);
        ...
        for (int i = 0; i < numCities; i++) {
            for (int j = i; j < numCities; j++) {
                if (i == j) {
                    ...
                    double pheromoneNumerator = Math.pow(pheromoneLevels[i][j], ALPHA);
                    double distanceNumerator = Math.pow((1 / (distances[i][j])), BETA);
                    probabilitiesList[i][j] = (pheromoneNumerator * distanceNumerator);
                    probabilitiesList[j][i] = (pheromoneNumerator * distanceNumerator);
                }
            }
        }
    }
     ...
     */
    private void gaUpdate() {
        //tournament selection
        Ant[] parentAnts = new Ant[NUM_ANTS];
        Random randomSelect = new Random();
         
        for (int i = 0; i < 8; i++) {
            //pick two random candidates from the initial population
            Ant candidateOne = antCollection[randomSelect.nextInt(NUM_ANTS)];
            Ant candidateTwo = antCollection[randomSelect.nextInt(NUM_ANTS)];
            
            //check which one has a higher fitness and add it to the new empty array
            if (candidateOne.getCurDistance() > candidateTwo.getCurDistance()) {
                parentAnts[i] = candidateOne;
            } else {
                parentAnts[i] = candidateTwo;
            }
        }
        
        //recombination/crossover
        if (crossoverType == "OX") {
            orderCrossover(parentAnts);
        } else if (crossoverType == "PMX") {
            partiallyMappedCrossover(parentAnts);
        }
    }
     ...
     */
    private void partiallyMappedCrossover(Ant[] parentAnts) {
        //sort the ants so that we can replace bottom ten with new children
        AntComparator comparator = new AntComparator();
        Arrays.sort(antCollection, comparator);
        
        Random randomSelect = new Random();
        for (int j = 0; j < 8; j += 2) {
            Ant antOne = parentAnts[j];
            Ant antTwo = parentAnts[j + 1];
            Ant childOne = new Ant(numCities);
            Ant childTwo = new Ant(numCities);
            ...
            int startEnd1 = 0;
            int startEnd2 = 0;
            while (startEnd1 == 0 || startEnd2 == 0 || startEnd1 == numCities - 1 || startEnd2 == numCities - 1 || startEnd1 == startEnd2) {
                startEnd1 = randomSelect.nextInt(numCities - 1);
                startEnd2 = randomSelect.nextInt(numCities - 1);
            }
            int start = Math.min(startEnd1, startEnd2);
            int end = Math.max(startEnd1, startEnd2);
            
            int position = start;
            while (position < end) {
                childOne.setTour(position, antTwo.getCurTour().get(position));
                childTwo.setTour(position, antOne.getCurTour().get(position));
                parentTwoMapping.put(antTwo.getCurTour().get(position), antOne.getCurTour().get(position));
                parentOneMapping.put(antOne.getCurTour().get(position), antTwo.getCurTour().get(position));
                position++;
            }
            
            int parentPosition = 0;
            while(parentPosition < numCities) {
                if (!childOne.getCurTour().contains(antOne.getCurTour().get(parentPosition)) && childOne.getCurTour().get(parentPosition) == -1) {
                    childOne.setTour(parentPosition, antOne.getCurTour().get(parentPosition));
                }
                if (!childTwo.getCurTour().contains(antTwo.getCurTour().get(parentPosition)) && childTwo.getCurTour().get(parentPosition) == -1) {
                    childTwo.setTour(parentPosition, antTwo.getCurTour().get(parentPosition));
                }
                parentPosition++;
            }
            
            for (int city = 0; city < numCities; city++) {
                if (childOne.getCurTour().get(city) == -1) {
                    int mapCheck = parentTwoMapping.get(antOne.getCurTour().get(city));
                    while (childOne.getCurTour().get(city) == -1) {
                        if (!childOne.getCurTour().contains(mapCheck)) {
                            childOne.setTour(city, mapCheck);
                        } else {
                            mapCheck = parentTwoMapping.get(mapCheck);
                        }
                    }
                }
                if (childTwo.getCurTour().get(city) == -1) {
                    int mapCheck2 = parentOneMapping.get(antTwo.getCurTour().get(city));
                    while (childTwo.getCurTour().get(city) == -1) {
                        if (!childTwo.getCurTour().contains(mapCheck2)) {
                            childTwo.setTour(city, mapCheck2);
                        } else {
                            mapCheck2 = parentOneMapping.get(mapCheck2);
                        }
                    }
                }
            }
            
            //set bottom ants to new children
            antCollection[NUM_ANTS - (j + 1)] = childOne;
            antCollection[NUM_ANTS - (j + 2)] = childTwo;
            
            //re-calculate distance
            childOne.calculateDistance(distances);
            childTwo.calculateDistance(distances);
        }
    }
     ...
     *      The parent ants that were chosen from the tournament selection for breeding. 
     */
    private void orderCrossover(Ant[] parentAnts) {
        //sort the ants so that we can replace bottom ten with new children
        AntComparator comparator = new AntComparator();
        Arrays.sort(antCollection, comparator);
        ...
        for (int j = 0; j < 8; j += 2) {
            ...
            Ant childOne = new Ant(numCities);
            Ant childTwo = new Ant(numCities);
            ArrayList<Boolean> parentOneCheck = new ArrayList<Boolean>();
            ArrayList<Boolean> parentTwoCheck = new ArrayList<Boolean>();
            for (int i = 0; i < numCities; i++) {
                parentOneCheck.add(i, true);
                parentTwoCheck.add(i, true);
            }
            ...
            int position = start;
            while (position < end) {
                childOne.setTour(position, antOne.getCurTour().get(position));
                childTwo.setTour(position, antTwo.getCurTour().get(position));
                parentOneCheck.set(antOne.getCurTour().indexOf(antTwo.getCurTour().get(position)), false);
                parentTwoCheck.set(antTwo.getCurTour().indexOf(antOne.getCurTour().get(position)), false);
                position++;
            }
            int positionTwo = end;
            int positionThree = end;
            int parentPosition = end;
            int counter = 0;
            while(counter < numCities) {
                if (parentTwoCheck.get(parentPosition) == true) {
                    childOne.setTour((positionTwo % numCities), antTwo.getCurTour().get(parentPosition));
                    positionTwo = (positionTwo + 1) % numCities;
                }
                if (parentOneCheck.get(parentPosition) == true) {
                    childTwo.setTour((positionThree % numCities), antOne.getCurTour().get(parentPosition));
                    positionThree = (positionThree + 1) % numCities;
                }
                parentPosition = (parentPosition + 1) % numCities;
                counter++;
            }
     ...
     */
    private double updatepheromoneLevels(Ant[] antCollection) {
        //decrease all pheromone levels first
        for (int legCol = 0; legCol < numCities; legCol++) {
            for (int legRow = legCol; legRow < numCities; legRow++) {
                pheromoneLevels[legRow][legCol] *= (1 - ROW);
                pheromoneLevels[legCol][legRow] *= (1 - ROW);
            }
        }
        
        double bestTour = Integer.MAX_VALUE;
        //deposit pheromone to all visited legs and also find current shortest tour
        for (int i = 0; i < NUM_ANTS; i++) {
            //get an ant
            Ant ant = antCollection[i];
            
            //iterate through the ants previous tour and for each leg, add to that legs pheromone level by
            // an amount of 1 divided by the total distance of that ants tour
            for (int leg = 0; leg < ant.getCurTour().size() - 1; leg++) {
                pheromoneLevels[ant.getCurTour().get(leg)][ant.getCurTour().get(leg + 1)] += (1 / ant.getCurDistance());
                pheromoneLevels[ant.getCurTour().get(leg + 1)][ant.getCurTour().get(leg)] += (1 / ant.getCurDistance());
            }
            pheromoneLevels[ant.getCurTour().get(ant.getCurTour().size() - 1)][ant.getCurTour().get(0)] += (1 / ant.getCurDistance());
            pheromoneLevels[ant.getCurTour().get(0)][ant.getCurTour().get(ant.getCurTour().size() - 1)] += (1 / ant.getCurDistance());
